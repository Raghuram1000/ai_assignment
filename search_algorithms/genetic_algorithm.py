...
def run_genetic(image="./image.jpg", truth="./gt.jpg", targetedVar=600, population_size=50, generations=100,mutation_rate=5):

    print(f'Population Size: {population_size}, Generations: {generations}, Mutation Rate: {mutation_rate}, Targeted Variance: {targetedVar}')
    blurred_image = cv2.imread(image, cv2.IMREAD_GRAYSCALE)
    image = cv2.imread(truth, cv2.IMREAD_GRAYSCALE)

    best_psf = genetic_algorithm(population_size, generations, mutation_rate, blurred_image, targetedVar)
    ...
    cv2.imwrite(f"outputV9.jpg", result_image)
