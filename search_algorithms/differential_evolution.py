...
def run_Differential(blur_img, clear_img, targetedVar=600):
    bounds = (-10, 10)

    blurred_image = cv2.imread(blur_img, cv2.IMREAD_GRAYSCALE)
    image = cv2.imread(clear_img, cv2.IMREAD_GRAYSCALE)
    #targetedVar = 600


    best_psf, best_cost = differential_evolution(objective_function, bounds, blurred_image, max_iter=40)
    ...
    cv2.imwrite(f"outputV10.jpg", result_image)
